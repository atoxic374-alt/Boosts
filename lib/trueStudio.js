...
// Discord uses this sentinel when there is no active account-level cooldown.
function isNoActivePremiumGuildSubscriptionCooldown(response) {
  const code = Number(response?.data?.code);
  const message = String(response?.data?.message || '');
  if (Number(response?.status) === 429) return false;
  return code === 10050 || /unknown premium server subscribe cooldown/i.test(message);
}
...
async function getPremiumGuildSubscriptionCooldown({ token, netOpts = {} }) {
  ...
  if (r.status >= 400) {
    // Discord returns JSON code 10050 when no account-level premium-server
    // subscription cooldown object exists. This is a valid empty state, not
    // an invalid token or a failed Nitro read. Slot-level cooldown_ends_at is
    // still checked separately by the caller.
    if (isNoActivePremiumGuildSubscriptionCooldown(r)) {
      return {
        ends_at: null,
        remaining: null,
        limit: null,
    ...
    throw new DiscordError('getPremiumGuildSubscriptionCooldown failed: ' + (r.data?.message || r.status), {
      ...
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0, rateLimit,
    });
  }
...
// Remove one applied Nitro boost from a guild so its slot can be applied elsewhere.
// This is a side-effecting operation and must be followed by a state refresh.
async function deletePremiumGuildSubscription({ token, guildId, subscriptionId, netOpts = {} }) {
  const gid = String(guildId || '').trim();
  const sid = String(subscriptionId || '').trim();
  if (!/^\d{5,30}$/.test(gid)) throw new DiscordError('Invalid source guild id', { code: 'INVALID_SOURCE_GUILD_ID' });
  if (!/^\d{5,30}$/.test(sid)) throw new DiscordError('Invalid premium subscription id', { code: 'INVALID_PREMIUM_SUBSCRIPTION_ID' });
  const r = await _req({
    method: 'DELETE',
    url: `${API}/guilds/${encodeURIComponent(gid)}/premium/subscriptions/${encodeURIComponent(sid)}`,
    token,
    netOpts,
  });
  if (r.status >= 400) {
    const rateLimit = rateLimitInfoFromResponse(r);
    throw new DiscordError('deletePremiumGuildSubscription failed: ' + (r.data?.message || r.status), {
      status: r.status, data: r.data,
      code: r.status === 429 ? 'RATE_LIMITED' : (r.data?.code || 'NITRO_UNBOOST_FAILED'),
      retryAfter: rateLimit.retryAfter || rateLimit.resetAfter || 0,
      rateLimit,
    });
  }
  return r.data && typeof r.data === 'object' ? r.data : { ok: true, status: r.status };
}
...
module.exports = {
  ...
  isNoActivePremiumGuildSubscriptionCooldown,
  applyPremiumGuildSubscriptions,
  deletePremiumGuildSubscription,
  acceptInvite,
  addBotToGuild,
  checkBotInGuild,
